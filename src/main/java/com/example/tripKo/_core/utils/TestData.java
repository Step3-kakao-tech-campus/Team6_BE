...
public class TestData implements CommandLineRunner {
  ...
  public void run(String... args) throws Exception {
    ...
    List<Address> addresses = Arrays.asList(
        Address.builder().zipCode("11111").roadName("RoadName1").buildingName("BuildingName1")
            .addressCategory(addressCategories.get(0)).build(),
        Address.builder().zipCode("22222").roadName("RoadName2").buildingName("BuildingName2")
            .addressCategory(addressCategories.get(0)).build(),
        Address.builder().zipCode("33333").roadName("RoadName3").buildingName("BuildingName3")
            .addressCategory(addressCategories.get(1)).build(),
        Address.builder().zipCode("44444").roadName("RoadName4").buildingName("BuildingName4")
            .addressCategory(addressCategories.get(0)).build(),
        Address.builder().zipCode("55555").roadName("RoadName5").buildingName("BuildingName5")
            .addressCategory(addressCategories.get(1)).build(),
        Address.builder().zipCode("66666").roadName("RoadName6").buildingName("BuildingName6")
            .addressCategory(addressCategories.get(2)).build()
    );
    addressRepository.saveAll(addresses);

    List<Place> places = Arrays.asList(
        Place.builder().name("정문토스트").summary("정문토스트는..").count(100).averageRating((float) 4.4)
            .address(addresses.get(0)).file(files.get(0)).placeType(PlaceType.RESTAURANT).build(),
        Place.builder().name("명물토스트").summary("명물토스트는..").count(200).averageRating((float) 4.2)
            .address(addresses.get(1)).file(files.get(1)).placeType(PlaceType.RESTAURANT).build(),
        Place.builder().name("2023 부산 국제 락 페스티벌").summary("부락페는..").count(300).averageRating((float) 4.3)
            .address(addresses.get(2)).file(files.get(3)).placeType(PlaceType.FESTIVAL).build(),
        Place.builder().name("2023 대동제").summary("대동제는..").count(200).averageRating((float) 4.0)
            .address(addresses.get(0)).file(files.get(3)).placeType(PlaceType.FESTIVAL).build(),
        Place.builder().name("Signiel Busan").summary("부산 시그니엘은..").count(100).averageRating((float) 4.5)
            .address(addresses.get(4)).file(files.get(6)).placeType(PlaceType.TOURIST_SPOT).build(),
        Place.builder().name("전포 카페 거리").summary("전포 카페 거리는..").count(200).averageRating((float) 4.3)
            .address(addresses.get(5)).file(files.get(7)).placeType(PlaceType.TOURIST_SPOT).build()
    );
    ...
    for (int i = 0; i < 6; i++) {
      ...
      contentsMenus.add(
          ContentsMenu.builder().contents(contents.get(i)).name(places.get(i).getName() + " 메뉴1").price(10000L)
              .characteristic("메뉴 특징1").file(files.get(9 + i)).description("메뉴 설명1").build());
      contentsMenus.add(
          ContentsMenu.builder().contents(contents.get(i)).name(places.get(i).getName() + " 메뉴2").price(10000L)
              .characteristic("메뉴 특징2").file(files.get(10 + i)).description("메뉴 설명2").build());
    ...
    List<Food> foods = Arrays.asList(
        Food.builder().name("Beef Tartare").keyword("BeefTartare sixtimes rawmeat").summary("육회는 ...")
            .foodCategory(FoodCategory.KOREAN).file(files.get(13)).description("육회 자세한 설명").build(),
        Food.builder().name("Gimbap").keyword("Gimbap kimbap futomaki norimaki").summary("김밥은 ...")
            .foodCategory(FoodCategory.KOREAN).file(files.get(14)).description("김밥 자세한 설명").build(),
        Food.builder().name("bibimbap").keyword("bibimbap KOREANMIXEDRICE").summary("비빔밥은 ...")
            .foodCategory(FoodCategory.KOREAN).file(files.get(15)).description("비빔밥 자세한 설명").build(),
        Food.builder().name("bibimnoodle").keyword("bibimnoodle bibimmyeon KOREANMIXEDNOODLE").summary("비빔국수는 ...")
            .foodCategory(FoodCategory.KOREAN).file(files.get(16)).description("비빔국수 자세한 설명").build()
    );
